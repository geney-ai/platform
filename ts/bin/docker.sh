...
validate_platform() {
    ...
    if ! docker image inspect "generic-ts/$SERVICE_NAME:latest" >/dev/null 2>&1; then
        echo "Error: Image generic-ts/$SERVICE_NAME:latest not found"
        exit 1
    fi

    # Check image platform
    PLATFORM=$(docker image inspect "generic-ts/$SERVICE_NAME:latest" --format '{{.Os}}/{{.Architecture}}')
...
push() {
    ...
    if ! docker tag generic-ts/$SERVICE_NAME:latest $REPOSITORY_URL:$ENV_NAME-$COMMIT_HASH ||
        ! docker tag generic-ts/$SERVICE_NAME:latest $REPOSITORY_URL:$ENV_NAME-latest; then
    ...
    if [[ "$ENV_NAME" == "production" ]] || [[ "$FORCE_LATEST" == 1 ]]; then
        if ! docker tag generic-ts/$SERVICE_NAME:latest $REPOSITORY_URL:latest; then
    ...
    if [[ "$TAG_AS_STAGING_LATEST" == 1 ]] && [[ ! "$SAFE_BRANCH_NAME" =~ ^(main|dev)$ ]]; then
        if ! docker tag generic-ts/$SERVICE_NAME:latest $REPOSITORY_URL:staging-latest; then
...
build)
    ...
    if [ -n "$PLATFORM" ]; then
        ...
        docker build $PLATFORM -t generic-ts/$SERVICE_NAME:latest -f apps/$SERVICE_NAME/Dockerfile .
    else
        docker build -t generic-ts/$SERVICE_NAME:latest -f apps/$SERVICE_NAME/Dockerfile .
    fi
    ;;
run)
    ...
    if [ -n "$PORT_MAPPING" ]; then
        ...
        docker run -it --rm $PORT_MAPPING generic-ts/$SERVICE_NAME:latest
    else
        docker run -it --rm generic-ts/$SERVICE_NAME:latest
    fi
    ;;
push)
